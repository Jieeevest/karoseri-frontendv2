...
import { useDeleteRequestMutation, useGetRequestsQuery } from "@/services/api";
...
export default function RequestItemsOverview() {
  ...
  const { data: requestData, isLoading, error } = useGetRequestsQuery(filter);
  ...
  useEffect(() => {
    const mappedData: any = items.map((item: any) => ({
      "": (
        <div className="flex justify-center items-center gap-2 cursor-pointer">
          <i
            ...
            onClick={() => router.push(`/workspace/requests/${item.id}/edit`)}
          />
          <i
      ...
      "Tanggal Kedatangan": item.arrivalDate,
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-base text-gray-600 pb-3">
              Mengelola daftar permintaan barang.
            </p>
          </div>
          <DefaultButton
            type="pill"
            ...
            onClick={() => router.push("/workspace/requests/add")}
          />
        </div>
        <DataTable
          title="Informasi Permintaan Barang"
          columns={columns}
          data={list}
          filter={filter}
          setFilter={setFilter}
